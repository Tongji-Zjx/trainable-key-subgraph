...
\[
d_{i,+}^{(m)}=\sum_j\max(A_{ij}^{(m)},0),\qquad
d_{i,-}^{(m)}=\sum_j\left|\min(A_{ij}^{(m)},0)\right|
\]
...
\[
r_{i,+}^{(m)}=\frac{d_{i,+}^{(m)}}
{d_{i,+}^{(m)}+d_{i,-}^{(m)}+\varepsilon},\qquad
r_{i,-}^{(m)}=\frac{d_{i,-}^{(m)}}
{d_{i,+}^{(m)}+d_{i,-}^{(m)}+\varepsilon}
\]

孤立节点的两个比例安全定义为 0。`degree` 继续保留，正负强度及比例作为
额外特征输入节点评分器，不用绝对值替代原始带符号邻接矩阵。
...
边评分器的显式输入维度固定为 4。端点的 signed 连接概况由节点评分器通过
13 维 signed 结构特征学习；原始社区编号和 ROI 身份不直接进入边 MLP。
...
assert positive_degree.shape == (N_bm,)
assert negative_degree.shape == (N_bm,)
assert delta_degree.shape == (N_bm,)
assert community_features.shape == (N_bm, 7)
assert node_features.shape == (N_bm, 13)
assert edge_features.shape == (N_bm, N_bm, 4)
...
`all_samples_exploratory` 协议：939 个原始文件中排除 1 个社区标签异常/空图
样本，其余 938 个样本全部进入显式的 `all`
...
坐标有效性不是纳入条件，坐标数值不输入模型；ROI 身份也不作为节点特征。
