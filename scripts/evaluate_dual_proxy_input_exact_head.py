"""Formally evaluate frozen D3 Proxy-Input Exact-Head on validation/test."""
...
from keysubgraph.analysis.dual_proxy_input_exact_head import (  # noqa: E402
...
def _validate_provenance(
    ...
    if (
        ...
        raise ValueError(
            "ProxyInput-ExactHead manifests use wrong splits"
        )
    keys = (
        "protocol_sha256",
    ...
    for key in keys:
        if validation_manifest[key] != test_manifest[key]:
            raise ValueError(
                "ProxyInput-ExactHead manifests disagree on {}".format(key)
            )
    if validation_manifest["protocol_sha256"] != protocol_sha:
        raise ValueError("ProxyInput-ExactHead protocol hash mismatch")
    if (
        validation_manifest["selector_checkpoint_sha256"] != selector_sha
    ):
        raise ValueError(
            "selector checkpoint does not match Exact-SGW manifests"
        )
    if validation_manifest["selection_mode"] != "learned":
        raise ValueError(
            "ProxyInput-ExactHead requires learned D3 selection"
        )
    if (
    ...
    if provenance.get("selector_checkpoint_sha256") != selector_sha:
        raise ValueError("Exact classifier uses a different selector")
    if provenance.get("sgw_scaler_sha256") != scaler_sha:
        raise ValueError("Exact classifier uses a different scaler")
...
def _validate_coverage(dataset, records, name):
    ...
    if assignments != cached:
        raise ValueError(
            "{} manifest does not exactly cover the frozen split".format(name)
        )
...
def _predict(
    ...
    with torch.no_grad():
        for cpu_batch in loader:
            ...
            logits = exact_model.sgw_auxiliary_head(scaler(proxy_raw))
...
def main():
    ...
    if args.batch_size < 1 or args.num_workers < 0:
        raise ValueError("invalid ProxyInput-ExactHead loader settings")
    output_dir = Path(args.output_dir).resolve()
    if output_dir.exists() and any(output_dir.iterdir()):
        raise FileExistsError(
            "ProxyInput-ExactHead output already exists"
        )
    ...
    evaluation = build_proxy_input_exact_head_evaluation(
    ...
    artifacts = write_proxy_input_exact_head_artifacts(
