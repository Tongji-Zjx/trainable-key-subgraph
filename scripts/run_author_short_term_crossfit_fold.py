"""Run one existing three-fold role with the author short-term branch."""
...
from keysubgraph.crossfit.author_short_term_runner import (  # noqa: E402
...
def _resume_train(command, artifact):
    if artifact.is_file():
        return command
    training = artifact.parent
    last_checkpoint = training / "last_checkpoint.pt"
    history = training / "history.json"
    if last_checkpoint.is_file():
        print("RESUME train: {}".format(last_checkpoint), flush=True)
        return list(command) + ["--resume", str(last_checkpoint)]
    if history.exists():
        raise RuntimeError(
            "partial author training has no resumable checkpoint: {}".format(
                training
            )
        )
    return command
...
def main():
    ...
    if not protocol.is_file():
        raise FileNotFoundError(
            "existing fold protocol is missing: {}".format(protocol)
        )
    commands = build_author_short_term_crossfit_fold_commands(
        PROJECT_ROOT,
        args.output_root,
        args.fold,
        args.profile,
    ...
    for stage, command, artifact in commands:
        if artifact.is_file():
            ...
            continue
        if stage == "train":
            command = _resume_train(command, artifact)
