"""Train one corrected neuralized S/V model on frozen hard-graph records."""
...
def parse_args():
    ...
    parser.add_argument("--quantile-loss-weight", type=float, default=0.02)
...
def main():
    ...
    if args.batch_size * args.gradient_accumulation_steps < 8 and not args.smoke:
        raise ValueError("formal corrected neural S/V training requires effective batch >= 8")
    maximum = 8 if args.smoke else None
    train = TheoryNeuralDataset(
        PROJECT_ROOT, args.train_manifest, args.scaler, max_samples=maximum
    )
    validation = TheoryNeuralDataset(
        PROJECT_ROOT, args.validation_manifest, args.scaler, max_samples=maximum
    )
    provenance_fields = (
        "protocol_sha256",
    ...
    if any(
        ...
        raise ValueError("corrected neural S/V train/validation provenance mismatch")
    train_loader = create_theory_neural_loader(
        train,
        args.batch_size,
        args.seed,
        True,
    ...
    validation_loader = create_theory_neural_loader(
        validation,
        args.batch_size,
        args.seed,
        False,
    ...
    model = NeuralizedSVClassifier(
        NeuralizedSVConfig(variant=args.variant, dropout=args.dropout)
    )
    config = TheoryNeuralTrainingConfig(
        ...
        quantile_loss_weight=(
            args.quantile_loss_weight if model.config.uses_static else 0.0
        ),
        transition_loss_weight=(
            args.transition_loss_weight if model.config.uses_evolution else 0.0
        ),
