"""Run one resumable fold of the frozen five-day SVG-v2 plan."""
...
from keysubgraph.crossfit.svg_v2_five_day_runner import (  # noqa: E402
...
def parse_args():
    ...
    parser.add_argument(
        "--candidates",
        nargs="+",
        choices=SVG_V2_ALL_CANDIDATES,
        default=list(SVG_V2_SCREEN_CANDIDATES),
    )
    parser.add_argument(
...
def _validate_source(args):
    ...
    required = [fold / "scaler.json"]
    splits = (
        ("train", "validation", "test")
        if args.mode == "confirmatory"
        else ("train", "validation")
    )
    required.extend(fold / "cache" / name / "manifest.json" for name in splits)
    missing = [str(path) for path in required if not path.is_file()]
    if missing:
        raise FileNotFoundError(
            "SVG-v2 source artifacts are missing: {}".format(missing)
        )
